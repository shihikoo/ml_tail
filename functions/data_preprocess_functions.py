...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  9 20:08:54 202
...
def find_filepath(datetime_str, dir_name='', file_append_name = 'identification', avg_hour = 6):
    ...
    cloak_filepath = dir_name+'cloak' + start_time_dt.apply(pd.Timestamp.strftime,format='%Y%m%d_%H%M%S') +'_to_' + end_time_dt.apply(pd.Timestamp.strftime,format='%Y%m%d_%H%M%S') + '_'+file_append_name+'.png'
    return cloak_filepath
...
#     cooked_data['kp_gt_2'] = cooked_data['kp'] > 2 
    ...
    cooked_data['cloak_filepath'] = find_filepath(cooked_data['datetime_str'], dir_name='plots/obeam_day/identification/', file_append_name = 'identification', avg_hour = 6)
